...
module.exports.getRepoImages = (config, ecr, ecs) => {
    var params = {
...
module.exports.deleteImages = (config, ecr, ecs) => images => {
    return new Promise(function(resolve) {
        ...
        if (config.DRY_RUN !== true) {
            if (imageTagsToDelete.length > 0) {
                ...
                ecr.batchDeleteImageAsync(params).then(function(deletions) {
                    resolve({
...
module.exports.filterOutActiveImages = (config, ecr, ecs) => eligibleForDeletion => {
    console.info("BEFORE FILTER:", eligibleForDeletion);

    return ecs
        .listTaskDefinitionsAsync({ status: "ACTIVE" })
        ...
        .then(function(taskDefs) {
            return new Promise(function(resolve) {
                Promise.map(taskDefs.taskDefinitionArns, function(taskDefinitionARN) {
                    ...
                    return ecs
                        .describeTaskDefinitionAsync({ taskDefinition: taskDefinitionARN })
                        .tap(() => Promise.delay(config.API_DELAY))
        ...
        .then(function(allImages) {
            ...
            const activeImages = _.chain(allImages).flattenDeep().uniq().value();
...
module.exports.filterImagesByDateThreshold = (config, ecr, ecs) => images => {
    ...
    return ecr.describeImagesAsync(params).then(function(response) {
...
// Filter out everything newer than some variable amount of days
// set via REPO_AGE_THRESHOLD (90 days by default)
module.exports.filterImagesByFirstN = (config, ecr, ecs) => images => {
